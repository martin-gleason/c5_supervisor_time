...
c5_time %>% group_by(first_name, last_name) %>% summarize(Hours_worked = sum(`Hours worked on project`)) %>% arrange(Hours_worked) %>% ggplot(aes(x = last_name, y = Hours_worked, fill = last_name)) + geom_bar(stat = "identity") + geom_hline(aes(yintercept = mean(c5_time$`Hours worked on project`)))
